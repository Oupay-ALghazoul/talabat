...
export class ListComponent implements OnInit {
  ...
  constructor( private router: Router, private modal: NgbModal ,   private user: UserServiceService ,
    ...
    this.translateMethod();
     ...
     if( this.permissionList != null){
      this.permissionList.some((x) => x === "create_warehouse")
        ? (this.showAdd = true)
        : (this.showAdd = false);
    }
    //  permission update
    if( this.permissionList != null){
      this.permissionList.some((x) => x === "update_warehouse")
        ? (this.showEdit = true)
        : (this.showEdit = false);
    }
    //  permission delete
    if( this.permissionList != null){
      this.permissionList.some((x) => x === "delete_warehouse")
  ...
  getWarehouse(){
    this.service.getWarehouse('true', this.page ).then(
      (data : any ) =>{
        console.log(data)
  ...
  delete(confirmForm)
  {
    ...
    this.service.deleteWarehouse( this.id_deleted_Item).then(
      res=> {
  ...
  ngOnInit() {
    ...
    this.breadcrumb = {
      'mainlabel': 'Warehouses Table',
    };
    this.breadcrumbAr = {
      'mainlabel': 'جدول المخازن',
    };
