...
export class ListComponent implements OnInit {
  ...
  constructor( private router: Router,  private modal: NgbModal, private user: UserServiceService  ,
    ...
    //  permission update
    if( this.permissionList != null){
      this.permissionList.some((x) => x === "update_deliveryOption")
        ? (this.showEdit = true)
        : (this.showEdit = false);
    }
    //  permission update
    if( this.permissionList != null){
      this.permissionList.some((x) => x === "delete_deliveryOption")
  ...
  getDeliveryOption() {
    this.service.getDeliveryOption().then((data : any) =>{
      this.result = data;
  ...
  ngOnInit() {
    ...
    this.breadcrumb = {
      'mainlabel': 'Delivery Options Table',
    };
    this.breadcrumbAr = {
      'mainlabel': 'جدول خيارات التوصيل المتاحة',
    };
  ...
  edit(id)
  {    
    this.router.navigate(['/deliveryOptions/edit/' + id]);
  }
  isItems = false;
  id_deleted_Item;
  addModal = null;
  ...
  delete(confirmForm)
  {
    ...
    this.service.deleteDeliveryOption( this.id_deleted_Item).then(
      res=> {
