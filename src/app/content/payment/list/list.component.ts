...
export class ListComponent implements OnInit {
  ...
  constructor( private router: Router,  private modal: NgbModal, private user: UserServiceService  ,
    ...
    this.translateMethod();
     ...
     if( this.permissionList != null){
      this.permissionList.some((x) => x === "create_payment")
        ? (this.showAdd = true)
        : (this.showAdd = false);
    }
    //  permission update
    if( this.permissionList != null){
      this.permissionList.some((x) => x === "update_payment")
        ? (this.showEdit = true)
        : (this.showEdit = false);
    }
    //  permission update
    if( this.permissionList != null){
      this.permissionList.some((x) => x === "delete_payment")
  ...
  getPayment() {
    this.service.getPayment().then((data : any) =>{
      this.result = data;
  ...
  ngOnInit() {
    ...
    this.breadcrumb = {
      'mainlabel': 'Payment Table',
    };
    this.breadcrumbAr = {
      'mainlabel': 'جدول  الدفع',
    };
  ...
  delete(confirmForm)
  {
    ...
    this.service.deletePayment( this.id_deleted_Item).then(
      res=> {
