...
export class ListComponent implements OnInit {
  ...
  constructor( private router: Router,  private modal: NgbModal, private user: UserServiceService  ,
    ...
    //  permission update
    if( this.permissionList != null){
      this.permissionList.some((x) => x === "update_outOfStock")
        ? (this.showEdit = true)
        : (this.showEdit = false);
    }
    //  permission update
    if( this.permissionList != null){
      this.permissionList.some((x) => x === "delete_outOfStock")
  ...
  outOfStock() {
    this.service.outOfStock().then((data : any) =>{
      this.result = data;
  ...
  ngOnInit() {
    ...
    this.breadcrumb = {
      'mainlabel': 'Out Of Stock Table',
    };
    this.breadcrumbAr = {
      'mainlabel': 'جدول حالات نفاذ الكمية',
    };
  ...
  edit(id)
  {    
    this.router.navigate(['/outOfStock/edit/' + id]);
  }
  isItems = false;
  id_deleted_Item;
  addModal = null;
  ...
  delete(confirmForm)
  {
    ...
    this.service.deleteOutOfStock( this.id_deleted_Item).then(
      res=> {
