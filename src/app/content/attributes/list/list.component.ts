...
export class ListComponent implements OnInit {
  ...
  constructor( private router: Router,  private modal: NgbModal, private user: UserServiceService  ,
    ...
    this.translateMethod();
     ...
     if( this.permissionList != null){
      this.permissionList.some((x) => x === "create_attribute")
        ? (this.showAdd = true)
        : (this.showAdd = false);
    }
    //  permission update
    if( this.permissionList != null){
      this.permissionList.some((x) => x === "update_attribute")
        ? (this.showEdit = true)
        : (this.showEdit = false);
    }
    //  permission update
    if( this.permissionList != null){
      this.permissionList.some((x) => x === "delete_attribute")
  ...
  getAttribute() {
    this.service.getAttribute().then((data : any) =>{
      this.result = data;
  ...
  ngOnInit() {
    ...
    this.breadcrumb = {
      'mainlabel': 'Product Attributes Table',
    };
    this.breadcrumbAr = {
      'mainlabel': 'جدول  خصائص المنتج',
    };
  ...
  delete(confirmForm)
  {
    ...
    this.service.deleteAttribute( this.id_deleted_Item).then(
      res=> {
