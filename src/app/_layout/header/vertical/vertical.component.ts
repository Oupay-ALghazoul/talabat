...
export class VerticalComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.service.profile().then(
      (data: User) => {
        this.username = data.full_name;
        this.image = data.image.low_quality
      })

    this.translateMethod()
    this.service.getSystemParam().then(res=>{this.systemParam = res})
  }
  showMore() {
    this.router.navigate(['/notification/allNotification']);
  }
  ...
  getNotifications(res) {
    ...
    this.notifications_length = this.notifications.length;
  }
  logout() {
  ...
  public changeLanguage(lang) {
    localStorage.setItem("selected", JSON.stringify(lang));
    this.service.updateLang()
  }
  public changeLanguageID(id) {

    localStorage.setItem("langId", JSON.stringify(id));

  }
  ngOnInit() {

    this.messagingService.customObservable.subscribe((res) => {
      this.getNotifications(res)
    }
    );
    ...
    if (localStorage.getItem('adminTalabat')) {
      this.currentUser = JSON.parse(localStorage.getItem('adminTalabat'));
    }
